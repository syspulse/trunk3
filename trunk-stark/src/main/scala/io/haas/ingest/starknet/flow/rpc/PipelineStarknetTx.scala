package io.haas.ingest.starknet.flow.rpc
...
import io.haas.ingest.starknet.Block
import io.haas.ingest.starknet.Tx
import io.haas.ingest.starknet.EventTx
import io.haas.ingest.starknet.StarknetJson._

import io.haas.ingest.starknet.flow.rpc._
import io.haas.ingest.starknet.flow.rpc.StarknetRpcJson._
  ...
  def transform(blk: RpcBlock): Seq[Tx] = {
    ...
    val txx = b.transactions.view.zipWithIndex.map{ case(tx,i) => {
      ...
      Tx(
        ...
        nonce = EthUtil.toLong(tx.nonce),
        from = tx.sender_address.getOrElse(""),
        fee = tx.max_fee.map(f => EthUtil.toBigInt(f)),
        typ = tx.`type`,
        ver = EthUtil.toLong(tx.version).toInt,
